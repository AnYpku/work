...
void xx::Loop()
{
   ...
   for(Int_t i=0;i<373;i++){
	   ...
	   h1[i] = new TH1D(name,name,1000,60,500);
   }
   Long64_t nbytes = 0, nb = 0,cut=0;
   ...
   if(!(photonet>30 && abs(photoneta)<1.4442)) continue; cut++; 
   ...
   cout<<"the biggest pweight = "<<*biggest_pweight<<"; position = "<<position<<endl;
          cout<<"pweight[476] = "<<pweight[476]<<"; pweight[849] = "<<pweight[849]<<"; pweight[0] = "<<pweight[0]<<endl;
          cout<<"========================"<<endl;
   vector_pweight.clear();
   }
   TFile* fhist = new TFile("histo.root","recreate");
   for(Int_t i=0;i<373;i++){h1[i]->Write();}
   fhist->Close();
   cout<<cut<<endl;
   /*TCanvas* c1[372];
   for(Int_t i=0;i<373;i++){
