...
//   nentries = 100000;
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      LEP = lep==13 && ptlep1 > 20. && ptlep2 > 20.&& abs(etalep1) < 2.4 &&abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep >70. && massVlep < 110 ;
      JET = jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7;
      for(Int_t j=0;j<6;j++){
          Photon_cut[j]= /*photon_pev[j]==1*/ photon_pevnew[j]==1 && photon_hoe[j]<0.0396  && photon_nhiso[j]<2.725 + 0.0148*photon_pt[j] + 0.000017*photon_pt[j]*photon_pt[j] && photon_phoiso[j]<2.571 + 0.0047*photon_pt[j]&&abs(photon_eta[j])<1.4442&&photon_pt[j]>25&&photon_pt[j]<400&&photon_drla[j]>0.7&&photon_drla[j]!=10; 
/*&&photon_sieie[j]<0.0102&&photon_sieie[j]>0.0052;*/
          if(Photon_cut[j]&&LEP&&JET){
