...
ofstream file3("./fitresult/info.txt");
...
TFile *f0 = new TFile(Form("../rootfiles/fdata_%0.f_%0.f.root",lowpt,highpt));
    ...
    Double_t nFakeStart = nData - nTrueStart;
	...
	RooRealVar nTrue("true number", "true number", nReal, 0, nData);
	RooRealVar nFake("fake number", "fake number", nFakeStart, 0, nData);
	...
	ofstream myfile(TString("./fitresult/txt/fakerate_") + Form("photon_pt%0.f_%0.f.txt", lowpt, highpt),ios::out);
	ofstream myfile1(TString("./fitresult/txt/fakerate_") + Form("pt%0.f_%0.f.txt", lowpt, highpt),ios::out);
	ofstream file(TString("./fitresult/txt/TrueNumber_") + Form("pt%0.f-%0.f.txt", lowpt, highpt),ios::out);
	ofstream file1(TString("./fitresult/txt/FakeNumber_") + Form("pt%0.f-%0.f.txt", lowpt, highpt),ios::out);
    ...
    char buffer[256];
	sprintf(buffer, "./fitresult/eps/pt%0.f-%0.f.eps",lowpt,highpt);
	c1->Print(buffer);
