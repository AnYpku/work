...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      //
      /*LEP = lep==13 && ptlep1 > 20. && ptlep2 > 20.&& abs(etalep1) < 2.4 &&abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep >70. && massVlep < 110 && drla>0.7;*/
      for(Int_t j=0;j<6;j++){
         if(name.Contains("outZA")==1)
           {Photon_cut[j]= /*photon_pev[j]==1*/ photon_pevnew[j]==1 && photon_hoe[j]<0.0396 && photon_chiso[j]<0.441 && photon_nhiso[j]<2.725 + 0.0148*photon_pt[j] + 0.000017*photon_pt[j]*photon_pt[j] && photon_phoiso[j]<2.571 + 0.0047*photon_pt[j]&&abs(photon_eta[j])<1.4442 && photon_isprompt[j]==1;} 
    
         if(name.Contains("outZJets")==1)
           {Photon_cut[j]= /*photon_pev[j]==1*/ photon_pevnew[j]==1 && photon_hoe[j]<0.0396 && photon_chiso[j]<0.441 && photon_nhiso[j]<2.725 + 0.0148*photon_pt[j] + 0.000017*photon_pt[j]*photon_pt[j] && photon_phoiso[j]<2.571 + 0.0047*photon_pt[j]&&abs(photon_eta[j])<1.4442 && photon_isprompt[j]==-1 && photon_pt[j]>10;} 

         if(name.Contains("outDMuon")==1)
           {Photon_cut[j]= /*photon_pev[j]==1*/ photon_pevnew[j]==1 && photon_hoe[j]<0.0396 && photon_chiso[j]<0.441 && photon_nhiso[j]<2.725 + 0.0148*photon_pt[j] + 0.000017*photon_pt[j]*photon_pt[j] && photon_phoiso[j]<2.571 + 0.0047*photon_pt[j]&&abs(photon_eta[j])<1.4442&&photon_pt[j]>10;} 
/*&&photon_sieie[j]<0.0102&&photon_sieie[j]>0.0052;*/
      ...
      for(Int_t k=0;k<ptnumber;k++){
         if(name.Contains("outZA")==1){
            if(photon_drla[position]>1 && photon_drla[position]<10 && 
               ...
               { h1[k]->Fill(photon_sieie[position],actualWeight);f++;}//fake
            }
         if(name.Contains("outZJets")==1){
	       ...
	       photon_pt[position]>lowpt[k] && photon_pt[position]<highpt[k]*/)
             { h2[k]->Fill(photon_sieie[position],actualWeight);t++;}//true
            }
         if(name.Contains("outDMuon")==1){
            ...
            if(photon_drla[position]>0.4 && photon_drla2[position]>0.4 &&
               ...
               Mva>81 && Mva<101 && photon_pt[position]>10/*&&
	       photon_pt[position]>lowpt[k] && photon_pt[position]<highpt[k]*/)
               { h4[k]->Fill(photon_sieie[position]);dt++;}//data true
           }
  ...
  /* ff = new TFile("./root/Three_template.root","recreate");
   ...
   if(name.Contains("outZA")){
       f1 = new TFile("./root/mcFake_template.root","recreate");
       f1->cd();
   ...
   if(name.Contains("outDMuon")){
       f3 = new TFile("./root/dataFake_template.root","recreate");
       f4 = new TFile("./root/dataTrue_template.root","recreate");
       f3->cd();
   ...
   if(name.Contains("outZJets")){
       f2 = new TFile("./root/mcTrue_template.root","recreate");
       f2->cd();
