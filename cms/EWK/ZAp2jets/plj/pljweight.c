...
Double_t weight[9];
 TFile* f1 = new TFile("cutLEP-plj_DMuon_histo.root");
 TFile* f2 = new TFile("cutLEP-outDMuon_histo.root");
 
 TH1F* h1 = (TH1F*)f1->Get("h_cutLEP-plj_DMuon");
 TH1F* h2 = (TH1F*)f2->Get("h_cutLEP-outDMuon");
void draw(){
...
void drawcom(){
 TCanvas* c2 = new TCanvas("c2","plj vs data",900,700);
// for(Int_t j=1;j<=9;j++){ h1->GetXaxis()->SetBinLabel(j,name[j-1]);}
 h1->SetLineColor(kRed);
 h1->SetLineWidth(3);
 h1->SetLineStyle(5);
 ...
 leg->AddEntry(h2, "data", "L");
 for(Int_t i=1;i<=9;i++){
     h1->SetBinContent(i,weight[i]*h1->GetBinContent(i));
    }
 h1->Draw();
 h2->Draw("Same");
 leg->Draw();
 c2->Print("./plj_com_data.eps");
 delete c2;
}
void open(Int_t i){
