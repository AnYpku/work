...
ofstream file1("scalefactor-cut.txt");
...
int unroll(){
    ...
    for(Int_t i=0;i<9;i++){
        ...
        t_ZA[p]= unroll(th2_ZA[p], mjj_bins, detajj_bins, 1,3,Form("hist_%d",i+1));//Form("%d central scale pdf variable",i+1));
        t_ZA[p]->SetLineWidth(3);
        t_ZA[p]->SetLineColor(i+1);
        for(Int_t j=1;j<=3;j++){ t_ZA[p]->GetXaxis()->SetBinLabel(j,name[j-1]);}
        t_ZA[p]->Scale(lumi*ZA_scale);
        t_ZA[p]->Draw("HIST");
//        t_ZA[i]->DrawNormalized("HIST");
        if(p==0) ll[p]->AddEntry(t_ZA[p],Form("%d central scale pdf variable",i+1));
        else     ll[p]->AddEntry(t_ZA[p],Form("%d scale pdf variable",i+1));
        ll[p]->Draw();
    ...
    cout<<"p = "<<p<<endl;
     TCanvas* can = new TCanvas("can","Mjj vs deltajj",900,600);
...
//          t_ZA[i]->Scale(lumi*ZA_scale);
          file1<<"histo "<<i+1<<"\t"<<t_ZA[0]->Integral()/t_ZA[m]->Integral()<<endl;
          cout<<"histo "<<i+1<<" "<<t_ZA[0]->Integral()/t_ZA[m]->Integral()<<endl;
          t_ZA[m]->Scale(t_ZA[0]->Integral()/t_ZA[m]->Integral());
          for(Int_t j=1;j<=3;j++){ t_ZA[m]->GetXaxis()->SetBinLabel(j,name[j-1]);}
          //t_ZA[i]->Draw("HIST,SAME");
          t_ZA[m]->Draw("HIST,SAME");
          l2->AddEntry(t_ZA[m],Form("%d  scale pdf variations",i+1));
         // delete t_ZA[i];
         // delete cc[i];
         m++;
       }
       l2->Draw();
       can->Print("hist-2d.eps");
