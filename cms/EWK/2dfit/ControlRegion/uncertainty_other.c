void open(Int_t i);
Int_t p=1;//the amount of bin
Int_t histo_number=8;//the histo_number equals the amount of pdf except the central value
Double_t central_value[1],nlo_value[8][1];//[the amount of pdf][the amout of bin]
...
for(Int_t j=0;j<p;j++){
  ...
  for(Int_t i=0;i<histo_number;i++){
      ...
      if(i==histo_number-1){
         ...
         uncertainty[j]=sqrt(sum/histo_number)/mean;
         }
      }
