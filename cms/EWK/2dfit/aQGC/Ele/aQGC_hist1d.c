...
void draw(Int_t num,TH1D* h[num]){
    ...
    for(Int_t i=0;i<9;i++){
        if(i==5 || i==7) continue;
        ll[p] = new TLegend(0.55,0.55,0.8,0.89);
...
//        h[i]->DrawNormalized("HIST");
     ...
     c1->SetGrid();
     TLegend *l2 = new TLegend(0.55,0.55,0.8,0.89);
     h[0]->SetTitle("Z and #gamma mass electron channel");
     h[0]->SetLineWidth(3);
     h[0]->GetYaxis()->SetRangeUser(0,12);
