TString plot="0826data";
TString rootfiles = "data";
TString sieiecut = "nosieiecut";
void style();
void histo();
...
void histo(){
 ...
 Double_t xhigh= 0.02;
  for(Int_t i=0;i<7;i++){
      h11[i]=new TH1D(Form("h11_pt%0.f-%0.f",lowpt[i],highpt[i]),"chiso<0.441",bin,0,xhigh);//Zjets && leding_photon && medium_cut && photon_chiso<1.416
      h21[i]=new TH1D(Form("h21_pt%0.f-%0.f",lowpt[i],highpt[i]),"chiso<0.441",bin,0,xhigh);//Zjets && leding_photon && medium_cut && photon_chiso<1.416
      h23[i]=new TH1D(Form("h23_pt%0.f-%0.f",lowpt[i],highpt[i]),"chiso<0.441 photon_isprompt=1",bin,0,xhigh);////Zjets && leding_photon && medium_cut && photon_chiso>2
      h13[i]=new TH1D(Form("h13_pt%0.f-%0.f",lowpt[i],highpt[i]),"chiso<0.441",bin,0,xhigh);////Zjets && leding_photon && medium_cut && photon_chiso>2
   ...
   for(Int_t k=0;k<7;k++){
       if(photon_chiso[position]<0.441&& *biggest_pt<highpt[k] && *biggest_pt>lowpt[k]) 
            {h1[k]->Fill(photon_sieie[position],scalef);m1[k] += scalef;
              if(photon_isprompt[position]!=1)  FakeNumber[k]+=scalef;
              if(photon_isprompt[position]==1)  TrueNumber[k]+=scalef;
               }//datamc
       for(Int_t j=0;j<21;j++){
       ...
       if(photon_isprompt[position]==1&&photon_chiso[position]<0.441&&*biggest_pt<highpt[k] && *biggest_pt>lowpt[k]) 
...
for(Int_t i=0;i<7;i++){
    ...
    ofstream myfile1(Form(sieiecut+"datanumber_pt%0.f-%0.f.txt",lowpt[i],highpt[i]));
    ...
    for(Int_t j=0;j<21;j++){
       ff1[i][j]= new TFile(Form(rootfiles+"_fake_pt%0.f-%0.f_chiso%0.f-%0.f.root",lowpt[i],highpt[i],lowchiso[j],highchiso[j]),"recreate");
       h12[i][j]->Write();
       h11[i]->Write();
       ...
       ofstream myfile_fake(Form(sieiecut+"mfakenumber_pt%0.f-%0.f_chiso%0.f-%0.f.txt",lowpt[i],highpt[i],lowchiso[j],highchiso[j]));
...
TFile *f2 = new TFile("../rootfiles/outZA_realphoton.root");     
...
for(Int_t i=0;i<7;i++){
 cout<<"pt range "<<lowpt[i]<<" ~ "<<highpt[i]<<endl;
ofstream myfile(Form(sieiecut+"truenumber_pt%0.f-%0.f.txt",lowpt[i],highpt[i]));
 ff2[i]   = new TFile(Form(rootfiles+"_photonreal_pt%0.f_%0.f.root",lowpt[i],highpt[i]),"recreate");
