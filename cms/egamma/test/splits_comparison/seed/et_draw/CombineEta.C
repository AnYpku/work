...
TFile*file;
void run(int j){
    TString split = Form("%.2f<eta<%.2f",etabins[j],etabins[j+1]);
    TString figname = n1 + Form("eta_%.2f_%.2f",etabins[j],etabins[j+1]);
    TFile* f1 = new TFile("./Splitfiles/"+n1+Form("_eta_%.2f_%.2f.root",etabins[j],etabins[j+1]));
   ...
   heta[1][i] = (TH1D*)f1->Get("heta_eghlt");  
   ...
   hnrgy[1][i] = (TH1D*)f1->Get("hnrgy_eghlt");    
   ...
   hphi[1][i] = (TH1D*)f1->Get("hphi_eghlt");      
   ...
   hdeta[1][i] = (TH1D*)f1->Get("hetain_eghlt");   
   ...
   hdphi[1][i] = (TH1D*)f1->Get("hphiin_eghlt");   
   ...
   hsieie[1][i] = (TH1D*)f1->Get("hsieie_eghlt");  ;
