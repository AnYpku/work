...
TFile*file;
void run(int j){
    TString split = Form("%0.f<et<%0.f",etbins[j],etbins[j+1]);
    TString figname = n1 + Form("et_%0.f_%0.f",etbins[j],etbins[j+1]);
    TFile* f1 = new TFile("./Splitfiles/"+n1+Form("_et_%0.f_%0.f.root",etbins[j],etbins[j+1]));
   ...
   het[1][i] = (TH1D*)f1->Get("hadem_eghlt");     
...
int CombineEt(){
    //Style();
    file = new TFile(n1+"et.root","recreate");
        for(int i=0;i<5;i++){
            run(i);
        }
