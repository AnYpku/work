...
void draw(TString name){
    TFile* file = new TFile(name+"et.root"); 
    TH1D* het_ele[6];
    TH1D* het_eghlt[6];
    vector<double> ymax;
    vector<int> fColors;
    ...
    fColors.push_back(kMagenta);
    fColors.push_back(kCyan);
    for(int i=0;i<5;i++){
        if(i==3)continue;
        het_ele[i]=(TH1D*)file->Get(Form("hdetEta_ele_%i",i));
        het_eghlt[i]=(TH1D*)file->Get(Form("heta_eghltUS_%i",i));
    }
    TLegend* l1 = new TLegend(0.6,0.6,0.9,0.9);
    TLegend* l2 = new TLegend(0.25,0.6,0.55,0.9);
    l1->SetTextSize(0.06);
    l2->SetTextSize(0.06);
    ...
    for(int i=0;i<5;i++){
        ...
        het_ele[i]->SetTitle("Eta comparison ;Eta;Normalized");
        het_ele[i]->SetLineColor(fColors[i]);
        het_ele[i]->SetLineWidth(2);
        ...
        het_eghlt[i]->SetLineWidth(2);
        het_eghlt[i]->SetLineStyle(1);
        het_eghlt[i]->GetYaxis()->SetTitleSize(0.07);
        het_eghlt[i]->GetYaxis()->SetTitleOffset(0.47);
        l1->AddEntry(het_ele[i],Form("reco %0.f<et<%0.f",etbins[i],etbins[i+1]));
        l2->AddEntry(het_eghlt[i],Form("HLT %0.f<et<%0.f",etbins[i],etbins[i+1]));
    ...
    line->Draw();
    c1->Draw();
    c1->SaveAs("./"+name+"_etaComp.eps");
    c1->SaveAs("./"+name+"_etaComp.pdf");
}
