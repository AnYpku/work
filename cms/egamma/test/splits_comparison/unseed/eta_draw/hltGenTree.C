...
void hltGenTree::Loop(TString name, int j)
{
   if (fChain == 0) return;
...
file = new TFile("./Splitfiles/"+name+Form("et_%0.f_%0.f.root",etbins[j],etbins[j+1]),"RECREATE");
het[0] = new TH1D("hclusEt_ele","clusEt",20,0,200);
het[1] = new TH1D("het_eghltUS","et",20,0,200);
Hadem[0] = new TH1D("hadem_ele","HoverE",20,0,0.1);
Hadem[1] = new TH1D("hadem_eghltUS","HoverE",20,0,0.1);
//heta[0] = new TH1D("hdetEta_ele","detEta",10,barreletabins[j],barreletabins[j+1]);
//heta[1] = new TH1D("heta_eghltUS","eta",10,barreletabins[j],barreletabins[j+1]);
heta[0] = new TH1D("hdetEta_ele","detEta",25,0,2.5);
heta[1] = new TH1D("heta_eghltUS","eta",25,0,2.5);
...
//      if( !(  (fabs(eleTruthEta)>1.566)&&(fabs(eleTruthEta)<2.5) ) ) continue;
        if(! (fabs(eleTruthEt)<etbins[j+1] && fabs(eleTruthEt)>etbins[j] ) ) continue;n1++;
        if(fabs(eleTruthEta)>1.4442 && fabs(eleTruthEta)<1.566) continue;
      ...
      Hadem[0]->Fill(hadem);Hadem[1]->Fill(egHLTUS_hadem);
