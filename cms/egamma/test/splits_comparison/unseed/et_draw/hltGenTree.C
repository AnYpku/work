...
file = new TFile("./Splitfiles/"+name+Form("eta_%.2f_%.2f.root",etabins[j],etabins[j+1]),"RECREATE");
...
Hadem[0]= new TH1D("Hadem_ele","HoverE",20,0,6);
Hadem[1]= new TH1D("Hadem_eghltUS","HoverE",20,0,6);
...
//      if( !(  (fabs(eleTruthEta)>1.566)&&(fabs(eleTruthEta)<2.5) ) ) continue;
        if(! (fabs(eleTruthEta)<etabins[j+1] && fabs(eleTruthEta)>etabins[j] ) ) continue;n1++;
