...
void draw(int i,TString n1,TString flag){
    ...
    TString Tag;
    if(flag_is_et)
        Tag = Form("%0.f<eleTruthEt<%0.f",bins[i],bins[i+1]);
    if(flag_is_eta)
        Tag = Form("%0.2f<eleTruthEta<%0.2f",bins[i],bins[i+1]);
    ...
    if (flag_is_et) {
    	...
    	figname = n1 + Form(flag+"_%0.f_%0.f_",bins[i],bins[i+1]);
    }
    if (flag_is_eta) {
    	ntulpename="Splitfiles/"+n1+Form(flag+"_%0.2f_%0.2f.root",bins[i],bins[i+1]);
    	figname = n1 + Form(flag+"_%0.2f_%0.2f_",bins[i],bins[i+1]);
    }
...
//    vector<vector<double>> maxegHLT(8,vector<double>(4));
    for(int i=0;i<number_hist;i++){
        if(i==3) continue;
        hadem[0][i] = (TH1D*)f1->Get(Form("hadem_ele_%i",i));
        hadem[1][i] = (TH1D*)f1->Get(Form("hadem_eghltUS_%i",i));  
        ...
        for(int n=0;n<number_hist;n++){
            if(n==3) continue;
        hele[0][n]=hadem[0][n];        hegHLT[0][n]=hadem[1][n];   
...
//    hele[0][0]->GetYaxis()->SetRangeUser(0,1);
 ...
 //       if(k==1) continue;
        ...
        if( hegHLT[k][0]->GetMaximumBin()*2>hegHLT[k][0]->GetNbinsX() ){
            if(Xtitle[k].Contains("Phi")==1){
                l1[k] = new TLegend(0.12,0.5,0.5,0.85);
                l2[k] = new TLegend(0.5,0.5,0.65,0.65);
            }
            else{
...
//        ymin=0;
        ...
        for(int i=0;i<number_hist;i++){
 		...
 		for (int i=0; i<number_hist; i++) {
            if(i==3) continue;
 			hratio[i]=(TH1D*)hegHLT[k][i]->Clone(Form("ratio_plot_%i",i));
        ...
        if(Xtitle[k].Contains("Phi")==1) rp->GetUpperRefYaxis()->SetRangeUser(0,ymax*2.4);
 		rp->GetLowerRefYaxis()->SetTitle("reco/HLT");
        //rp->GetLowerRefXaxis()->SetRangeUser(0,(xmax+8*hele[k][0]->GetBinWidth(1)));	
//        rp->GetLowerRefYaxis()->SetRangeUser(-1,6);	
...
//            hratio[i]->GetYaxis()->SetRangeUser(-5,5);
    	...
    	TString plot_path="Splitfiles/plots/pdf/"+figname;
