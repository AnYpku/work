...
{
   if (fChain == 0) return;
//file = new TFile("./Splitfiles/"+name+Form("et_%0.f_%0.f-eta_%.2f_%.2f.root",etbins[i],etbins[i+1],barreletabins[j],barreletabins[j+1]),"RECREATE");
//file = new TFile("./Splitfiles/"+name+Form("et_%0.f_%0.f-eta_%.2f_%.2f.root",etbins[i],etbins[i+1],endcapetabins[j],endcapetabins[j+1]),"RECREATE");
file = new TFile("./Splitfiles/splits/"+name+Form("et_%0.f_%0.f-eta_%.2f_%.2f.root",etbins[i],etbins[i+1],etabins[j],etabins[j+1]),"RECREATE");
het[0] = new TH1D("hclusEt_ele","clusEt",20,0,400);
het[1] = new TH1D("het_eghltUS","et",20,0,400);
heta[0] = new TH1D("hdetEta_ele","detEta",10,0,2.5);
heta[1] = new TH1D("heta_eghltUS","eta",10,0,2.5);
Hadem[0] = new TH1D("hadem_ele","HoverE",10,0,5);
Hadem[1] = new TH1D("hadem_eghltUS","HoverE",10,0,5);
...
tree->Branch("eleTruthNrgy",&eleTruthNrgy,"eleTruthNrgy/F");*/
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if(clusEt>200) clusEt =199; if(egHLTUS_et>200) egHLTUS_et =199;
...
//      if(! (fabs(eleTruthEta)<endcapetabins[j+1] && fabs(eleTruthEta)>endcapetabins[j] && eleTruthEt<etbins[i+1]&&eleTruthEt>etbins[i]) ) continue;n1++;

//      if( !(  (fabs(eleTruthEta)>1.566)&&(fabs(eleTruthEta)<2.5) ) ) continue;
        if(! (fabs(eleTruthEta)<etabins[j+1] && fabs(eleTruthEta)>etabins[j] && eleTruthEt<etbins[i+1]&&eleTruthEt>etbins[i]) ) continue;n1++;
      Hadem[0]->Fill(hadem);Hadem[1]->Fill(egHLTUS_hadem);hnrgy[0]->Fill(clusNrgy);hnrgy[1]->Fill(egHLTUS_nrgy);
