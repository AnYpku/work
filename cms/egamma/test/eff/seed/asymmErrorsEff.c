...
for(int i = 0;i<num;i++){
	...
	xerror_SBlow[i]=h1->GetBinWidth(i+1)/2;
        ...
        if(name.Contains("Nrgy")&&eS_barrel->GetEfficiency(i+1)==0){
		eff_SB[i]= 0;yerror_SBlow[i]=0;yerror_SBhigh[i]=0;
        }
	...
	xerror_SElow[i] = h2->GetBinWidth(i+1)/2;
        ...
        if(name.Contains("Nrgy")&&eS_endcap->GetEfficiency(i+1)==0){
		eff_SE[i]= 0;yerror_SElow[i]=0;yerror_SEhigh[i]=0;
	}
	...
	xerror_USBlow[i]=h1->GetBinWidth(i+1)/2;
        ...
        if(name.Contains("Nrgy")&&eUS_barrel->GetEfficiency(i+1)==0){
                eff_USB[i]= 0;yerror_USBlow[i]=0;yerror_USBhigh[i]=0;
        }
	...
	xerror_USElow[i] = h2->GetBinWidth(i+1)/2;
        ...
        if(name.Contains("Nrgy")&&eUS_endcap->GetEfficiency(i+1)==0){
                eff_USE[i]= 0;yerror_USElow[i]=0;yerror_USEhigh[i]=0;
        }
...
if(filename.Contains("200"))
	effS_barrel->SetTitle("seed & unseed efficiency vs "+ name + " "+tag2);
else    effS_barrel->SetTitle("seed & unseed efficiency vs "+ name + " "+tag1);
cout<<"OK"<<endl;
...
if(filename.Contains("200"))
   {
	   c1->Print("./eff_figs/"+tag2+name+nameS+".pdf");
	   c2->Print("./eff_figs/"+tag2+name+nameUS+".pdf");
	   c3->Print("./eff_figs/"+tag2+name+"_comp.pdf");
   }
else  {
	c1->Print("./eff_figs/"+tag1+name+nameS+".pdf"); c2->Print("./eff_figs/"+tag1+name+nameUS+".pdf");
	c3->Print("./eff_figs/"+tag1+name+"_comp.pdf");
      }
//delete c1;delete c2;
}
