...
file = new TFile("./Splitfiles/"+name+Form("et_%0.f_%0.f-eta_%.2f_%.2f.root",etbins[i],etbins[i+1],etabins[j],etabins[j+1]),"RECREATE");
...
//      if( !(  (fabs(eleTruthEta)>1.566)&&(fabs(eleTruthEta)<2.5) ) ) continue;
        ...
        het[0]->Fill(clusEt);het[1]->Fill(egHLTUS_et);
        hnrgy[0]->Fill(clusNrgy);hnrgy[1]->Fill(egHLTUS_nrgy);
        heta[0] ->Fill(detEta);heta[1] ->Fill(egHLTUS_eta);
        hphi[0]->Fill(detPhi);hphi[1]->Fill(egHLTUS_phi);
