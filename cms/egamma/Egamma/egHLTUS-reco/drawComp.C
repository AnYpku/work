...
//    TString split = Form("%0.f<et<%0.f and %.2f<eta<%.2f",etbins[i],etbins[i+1],endcapetabins[j],endcapetabins[j+1]);
//    TString figname = n1 + Form("et_%0.f_%0.f_eta_%.2f_%.2f",etbins[i],etbins[i+1],endcapetabins[j],endcapetabins[j+1]);
//    TFile* f1 = new TFile("./Splitfiles/"+n1+Form("et_%0.f_%0.f-eta_%.2f_%.2f.root",etbins[i],etbins[i+1],endcapetabins[j],endcapetabins[j+1]));
...
/*        if(elemax > egHLTmax ){
		...
		l1[i]->Draw();
//		c1[i]->Draw();
		c1[i]->Print("./fig/"+figname+name+".pdf");
        delete c1[i];
	}
}
void Style(){
