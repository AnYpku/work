...
void newana1(TString process="rec",TString fit="CB",Double_t mlow=105,Double_t mhigh=140,Bool_t idebug=kFALSE){

//void newana1(TString process="rec",Double_t mlow=105,Double_t mhigh=140,Bool_t idebug=kTRUE){
  SetPKUStyle();
  ...
  // Construct signal
  RooRealVar mean("mean","mean",125,mlow,mhigh) ;
  ...
  {
  //RooRealVar mean1("mean1","Mean1",125,mlow,mhigh);
  RooRealVar gamma("gmma","gmma",5,0.1,10);
  RooBreitWigner sign("bw","bw PDF",x,mean,gamma);
  }
if(process=="gen")
{    RooRealVar mean1("mean1","mean1",90,80,105) ;
    ...
    /*
    RooRealVar mean1("mean1","mean1",125,mlow,mhigh) ;
    RooRealVar sigma1("sigma1","sigma1",1,1,10) ;
    if(fit=="CB"){
...
//RooAddPdf model("model","model",RooArgList(sign,sign1),RooArgList(nsig,nsig1));
  RooAddPdf model("model","model",sign,nsig);
  }

  RooFitResult* r =  model.fitTo(data,Save(),Minos(kTRUE),NumCPU(32));

  ////draw the fitted results
  if(process=="rec")
  RooPlot* xframe = x.frame(Bins(50),Title("llgg_rec;invariant mass of Higgs [GeV];Counts/0.2GeV")) ;
  else if(process=="gen") RooPlot* xframe = x.frame(Bins(50),Title("llgg_gen;invariant mass of Higgs [GeV];Counts/0.2GeV")) ;
  ...
  //RooPlot *sig_2= model.plotOn(xframe,Components("sign1"),LineWidth(3),LineStyle(3),LineColor(kBlue));
  if(process=="rec")RooPlot *bkg_1=model.plotOn(xframe,Components("fbkg"),LineWidth(3),LineStyle(2),LineColor(kBlue));
  if(process=="rec")RooPlot *model_1=model.plotOn(xframe,LineColor(kGreen));
  ...
  //txt(0.2,d1,Form("Nbkg=%.1f#pm%.1f",nbkg.getVal(),nbkg.getError()));    d1-=dd;
  ...
  if(fit=="bw") leg->AddEntry(c_sig,"signal(bw)","L");
