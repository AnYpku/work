...
void newana1(TString process="gen",TString fit="gaus",Double_t mlow=124.99999,Double_t mhigh=125.000006,Bool_t idebug=kFALSE){
...
//void newana1(TString process="gen",Double_t mlow=120,Double_t mhigh=130,Bool_t idebug=kTRUE){
  ...
  RooRealVar sigma("sigma","sigma",0.00000001,0.00000001,10) ;
  if(fit=="CB"){
...
//RooAddPdf model("model","model",RooArgList(sign,sign1),RooArgList(nsig,nsig1));
  ...
  //if(process=="rec")RooPlot *bkg_1=model.plotOn(xframe,Components("sign1"),LineWidth(3),LineStyle(2),LineColor(kBlue));
  //if(process=="rec")
  ...
  txt(0.2,d1,Form("Sigma=%.8f#pm%.8f",sigma.getVal(),sigma.getError()));    d1-=dd;
