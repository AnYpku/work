...
#include "C:\Users\atrisovi\LHCbMasterclass\src\D0_Lifetime\launcher.h"

Frame::Frame():fHistCanvas(0),PI(3.141592653589793238462){
int a=1;
}

Frame::Frame(const int lang, const char* user, Int_t h, launcher *lstart): fHistCanvas( 0 ), PI(3.141592653589793238462) {
    TEveManager::Create();
	printf("%s\n", user);
	user1=Form("%s",user);
	printf("%s\n", user1);
	...
	//gEve->
    v->ColorSet().Background().SetColor(kWhite); //pozadina
...
void Frame::makeGui() {
	...
	//gEve->GetBrowser()->GetMainFrame()->MapWindow();
	...
	//browser->SetMWMHints(kMWMDecorAll, kMWMFuncAll ^ kMWMFuncClose, kMWMInputModeless);
   ...
   TGLabel *fLabel926 = new TGLabel(fGroupFrame717,"MeV/c²");  
 ...
 fTextEntry1337 = new TGTextEntry(fGroupFrame735, new TGTextBuffer(15),-1,uGC->GetGC(),ufont->GetFontStruct(),kSunkenFrame | kDoubleBorder | kOwnBackground);
   ...
   fTextButton834 = new TGTextButton(fGroupFrame1395,"Exit");
...
void Frame::SaveHist(){
	...
	//printf("Ime: %s",histName1);
	TFile *saveH=new TFile(histName1,"RECREATE");
	a->Write();
	//saveH->ls();
	saveH->Close();
...
void Frame::readTracks() {
	...
	for (int a=0; a<size; a++)
	{
		...
		float index=pointsArray->GetX(0)+pointsArray->GetY(0)+pointsArray->GetX(p-1);
		particles[index] = *t;
		//printf("%f\n",index);
...
void Frame :: Calculate(){
	float a=(myParticle1.E + myParticle2.E)*(myParticle1.E + myParticle2.E);
    float b=(myParticle1.px + myParticle2.px)*(myParticle1.px + myParticle2.px);
	float c=(myParticle1.py + myParticle2.py)*(myParticle1.py + myParticle2.py);
	float d=(myParticle1.pz + myParticle2.pz)*(myParticle1.pz + myParticle2.pz);
	res=sqrt(a-b-c-d);
...
void Frame :: DE()
{
	...
	e1="Du kannst nicht mehr als zwei Teilchen auswählen.";
	e2="Dieses Teilchen ist bereits ausgewählt.";
	h1="Das Histogram wurde erfolgreich gespeichert.";
	h2="Wenn du die Ansicht rotieren willst, drücke STRG oder STRG + SHIFT um präziser zu arbeiten.";
	e3="Die Masse liegt nicht im korrekten Interval. Versuche es erneut.";
	e4="Du hast bereits ein Massen-Histogram hinzugefügt.";
	h3_a="Herzlichen Glückwunsch!"; 
	h3_b="Du hast die MasterclassDisplay-Aufgabe erfolgreich gelöst.";
...
void Frame :: FR()
{
	...
	fLabel842->SetText ("Numéro d'evenement:");           //12
