...
class Frame {
	public:
		...
		Frame(const int lang, const char* user,TString pathSave2, Int_t h, launcher *lstart);
		virtual ~Frame();
		void makeGui();	
		//event managment 
		/***
	...
	private:
		...
		TObjArray *array;
		const double PI;  
		std::map <int, LbMclassTrack> particles;
	  ...
	  //messages in application - for translation
	  TString e1;
	  TString e2;
	  TString h1;
	  TString e3;
