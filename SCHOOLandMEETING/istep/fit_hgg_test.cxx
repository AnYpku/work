...
//	RooDataHist data("data","data", hgg, h1); //binned fit
    RooDataSet data("data", "data", t1,hgg);//unbinned fit
//========== hgg pdf =============
    RooRealVar mean1("mean_GS",   "", 124,120,130);
    RooRealVar sigma1("sigma_GS", "", para[11],   0.5, 5);
    RooGaussian gauss1_pdf("gaussian", "", hgg, mean1, sigma1);

    RooRealVar mean2("mean2_GS",   "",135,130,140);
    RooRealVar sigma2("sigma2_GS", "", para[6],   0.2, 5);
    RooGaussian gauss2_pdf("gaussian2", "", hgg, mean2, sigma2);

    RooRealVar Nf("Nfrac",  "",   0, 1.0);
    RooAddPdf sig_pdf("sum of sig","gauss + CB",RooArgList(gauss1_pdf,gauss2_pdf),RooArgList(Nf));
    ...
    RooAddPdf sum("sum","signalPDF + exp PDF",RooArgList(sig_pdf, expo),RooArgList(N1,N3));
...
//============== plot hgg ============
    ...
    sum.plotOn(frame_hgg, Name("signal"),Components(sig_pdf), LineColor(kGreen));
    sum.plotOn(frame_hgg, Name("bkg"),Components(expo), LineColor(kBlue),LineStyle(2));
	sum.paramOn(frame_hgg, Parameters(RooArgSet(N1,N3,Nf,mean1,sigma1)), RooFit::Layout(0.6, 0.98));
    ...
    frame_hgg->SetXTitle("M(Higgs) (GeV/c^{2})");
...
//     frame_hgg->addObject(myar5);
     frame_hgg->addObject(leg1);
        ...
        frame_hgg->Draw();
        pt2->Draw();
	  c_hgg->Print("fit_hgg-test1.pdf");

//        hgg.setRange("MassWindow",0.11497,0.15497);
//        RooAbsReal* nsig_masswindow = gauss1_pdf.createIntegral(hgg,NormSet(hgg),Range("MassWindow"));
//        Int_t Nsig_masswindow = N1.getVal()*(nsig_masswindow->getVal());

//        hgg.setRange("AllRegion",0.05,0.22);
//        RooAbsReal* nsig_allregion = gauss1_pdf.createIntegral(hgg,NormSet(hgg),Range("AllRegion"));
