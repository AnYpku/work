...
void newana1(TString process="gen",TString fit="CB",Double_t mlow=120,Double_t mhigh=130,Bool_t idebug=kFALSE){

//void newana1(TString process="rec",Double_t mlow=120,Double_t mhigh=130,Bool_t idebug=kTRUE){
  SetPKUStyle();
  TH1F *h = new TH1F("h",";invariant mass of Higgs [GeV];Counts",1400,0,140); 
  
  RooRealVar x("x","x",mlow,mhigh) ;
  
  RooDataSet data("data","",x);
  if(process=="rec")
  TFile *f = new TFile("newsscc_gen.root");
  else if(process=="gen") TFile *f = new TFile("newsscc_gen.root");
  TTree *t=new TTree("t","t");
  f->GetObject("t1",t);
  Double_t H[4];
  t->SetBranchAddress("H[4]",H);
  Long64_t nentries = t->GetEntries();
  Long64_t ntotal=0;
  for (Long64_t i=0; i<nentries;i++) {
  t->GetEntry(i);
  if(H[3]>=mlow&&H[3]<=mhigh){
  x=H[3];
  data.add(x);
  ntotal++;}
  h->Fill(H[3]);
  ...
  RooFitResult* r =  model.fitTo(data,Save(),Minos(kTRUE));

  ////draw the fitted results
  if(process=="rec")
  RooPlot* xframe = x.frame(Bins(50),Title("sscc_rec;invariant mass of Higgs [GeV];Counts/0.2GeV")) ;
  else if(process=="gen") RooPlot* xframe = x.frame(Bins(50),Title("sscc_gen;invariant mass of Higgs [GeV];Counts/0.2GeV")) ;
