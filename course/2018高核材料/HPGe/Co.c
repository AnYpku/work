...
while(nlines<1024){
   ...
   x2[nlines]=nlines;
   if(nlines<850)
     {channel_Co.push_back(0);}
   else  {channel_Co.push_back(y[nlines]);}
   h1->SetBinContent(nlines,y[nlines]);
   nlines++;
   }
int size=channel_Co.size();
biggest = max_element(begin(channel_Co),end(channel_Co));
int position = distance( begin(channel_Co),biggest);
f1.close();
f2.close();
cout<<"the biggest"<<*biggest<<endl;
cout<<"position = "<<position<<endl;
Double_t max=*biggest;
cout<<" max = "<<max<<endl;
Double_t bin = h1->FindBin(0.5*max);
cout<<"bin number = "<<bin<<endl;
cout<<"bin content = "<<h1->GetBinContent(h1->FindBin(0.5*max))<<endl;
...
h1->SetTitle("^{60}Co energy spectrum;channel;count");
