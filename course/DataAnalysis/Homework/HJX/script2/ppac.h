...
// Mon Mar 12 14:14:55 2018 by ROOT version 6.11/02
// from TTree t1//rarf/w/r04/chenrj/root/runmust20083.root
// found on file: runmust20083.root
//////////////////////////////////////////////////////////
...
public :
  ...
  virtual void     Show(Long64_t entry = -1);
	...
	// User's data members
	Float_t         kx,bx,ky,by;
	Float_t         PPAC1Position[2][4];
	Float_t         PPAC2Position[2][4];
	Float_t         PPAC3Position[4];
	Float_t         targetPosition[3];
	Float_t         rotatedTargetPosition[3];

	Long64_t 				numberOfValidTraceX;
	Long64_t 				numberOfValidTraceY;

	TH2F           *hXTrace;
	TH2F           *hYTrace;
	TH2F           *hTargetDistribution;
	//TH2F           *hRotatedTargetDistribution;
	TH3F           *hRotatedTargetDistribution;
	TCanvas        *cTrace;
	TLine          *lTarget;
	TLine          *lPPAC1;
	TLine          *lPPAC2;
	...
	// User's function members
	void Initial();
	void Loop(TTree *fopt);
	//void ResetVal();
	//void MappingPPAC();
	void ExtractData();
	void BranchOpt(TTree *fopt);
	void Track(Long64_t &fnumberOfValidTraceX,Long64_t &fnumberOfValidTraceY);
...
ppac::ppac(TTree *tree) : fChain(0) 
{
	...
	if (tree == 0) {
		TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("runmust20083.root");
		if (!f || !f->IsOpen()) {
			f = new TFile("runmust20083.root");
		}
		f->GetObject("t1",tree);
	}
