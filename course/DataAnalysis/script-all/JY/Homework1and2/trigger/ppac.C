...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   Long64_t nentries = fChain->GetEntriesFast();
	 ...
	 fChain->SetBranchStatus("GATCONF",1);

	 TCanvas *c = new TCanvas("c","c",400,400);
	 TH2F*  htar = new TH2F("htar","Distribution on Target",100,-50,50,100,-50,50);

	 TFile* opf = new TFile("PPACF8.root","recreate");
	 ...
	 Int_t tTrig[16]; //trigger information:0~7 for MUST2, 11 for beam
	 ...
	 opt->Branch("tTrig",tTrig,"tTrig[16]/I");
...
void ppac::TrigSelection(Int_t tGATCONF, Int_t tTrig[16]) {
	
	Int_t icoin = tGATCONF;
	Int_t ibit[16], imask;
