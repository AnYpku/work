...
# Download sandbox
sandbox_name="sandbox-CMSSW_9_3_13-1980360.tar.bz2"
wget --no-check-certificate --progress=bar "http://stash.osgconnect.net/+jiexiao/${sandbox_name}" || exit_on_error $? 150 "Could not download sandbox."
...
cd $CMSSW_BASE/src/MY/PROJECT/python
...
# Execute ROOT script
cmsenv
cmsRun sherpa_13TeV_DYJets_NLO_MASTER_cff_py_GEN.py  
...
# Clean up
cd -
mv $CMSSW_BASE/src/MY/PROJECT/python/*.yoda .
rm $sandbox_name
